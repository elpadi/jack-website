...
define('lib/ui/SectionSwitcher/ArrowNav',['lib/fn/bind'], function(bind) {
	return {
		init: function($container) {
			this.arrows = {
				$next: $container.find('.section-switcher__nav__next'),
				$prev: $container.find('.section-switcher__nav__prev')
			};
			$container.on('click', '.section-switcher__nav a', function(e) {
				this.trigger('arrowclicked', e);
			}.bind(this));
		},
		...
		sectionswitched: function(newIndex) {
			...
			this.$container.toggleClass('section-switcher--left-edge', prevIndex === false)
				.toggleClass('section-switcher--right-edge', nextIndex === false);
			this.arrows.$prev.attr('href', '#' + this.$elements.eq(prevIndex).attr('id'));
			this.arrows.$next.attr('href', '#' + this.$elements.eq(nextIndex).attr('id'));
		}
	};
});
...
define('lib/dom/trackers/scroll',['jquery'], function(jquery) {
	...
	$(window).on('scroll', function(e) {
		...
		for (var i in fns) fns[i](scrollTop);
	});
	return function onScroll(fn) {
		fn(document.body.scrollTop);
		fns.push(fn);
	};
});

define('lib/dom/absolute-fixed',['jquery','lib/dom/trackers/scroll'], function(jquery, scrollTracker) {
	$('.absolute-fixed').each(function(i, el) {
		var $el = $(el);
		var offset = $el.offset();
		var minTop = $el.data('min-top');
		var css = { position:'fixed', 'top': minTop + 'px', 'left': offset.left + 'px' };
		var noCss = { position:'', 'top': '', 'left':'' };
		var onScroll = function(scrollTop) {
			$el.css(scrollTop >= (minTop + offset.top) ? css : noCss);
		};
		if (typeof(minTop) === 'number' && minTop > 0) {
			scrollTracker(onScroll);
		}
	});
});
...
define('lib/dom/window-height',['jquery','lib/dom/trackers/resize'], function(jquery, resizeTracker) {
	$('.window-height').each(function(i, el) {
		var $el = $(el);
		var onResize = function(wWidth, wHeight) {
			$el.css('height', wHeight + 'px');
		};
